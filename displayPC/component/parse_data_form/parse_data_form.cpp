#include "parse_data_form.h"
#include "parse_data_form_ui.h"

ParseDataForm::ParseDataForm(QWidget *parent) : QWidget(parent),
...
bool ParseDataForm::initTruckParam()
{
  ...
  for (size_t i = 0; i < trucks_name.size(); i++)
  {
    ...
    if (!isValid)
    {
      QMessageBox::warning(this, "warning", QString("车辆参数不全"));
      continue;
    }
    ...
    if (temp.ip.size() != 4)
    {
      QMessageBox::warning(this, "warning", QString::fromStdString(temp.name) + QString("Wrong IP address"));
      continue;
    }

    if (temp.lf.size() != 6)
    {
      QMessageBox::warning(this, "warning", QString::fromStdString(temp.name) + QString("Param of front lidar is wrong"));
      continue;
    }

    if (temp.lf.size() != 6)
    {
      QMessageBox::warning(this, "warning", QString::fromStdString(temp.name) + QString("Param of back lidar is wrong"));
      continue;
    }
...
void ParseDataForm::on_save_param_clicked()
{
